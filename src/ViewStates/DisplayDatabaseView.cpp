...
DisplayDatabaseView::DisplayDatabaseView(
    const ViewStateFactory &viewStateFactory, int height, int width)
  ...
  window = newwin(winSize->y, winSize->x, scrSize->centeredYBy(*winSize),
                  scrSize->centeredXBy(*winSize));
  keypad(window, TRUE);
...
std::shared_ptr<IViewState> DisplayDatabaseView::nextState(TuiView &view) {
  ...
  for (size_t i = 0; i < dataRows.size(); ++i) {
    strRows.push_back(dataRows[i].date + "    " + dataRows[i].name + "    " +
                      std::to_string(dataRows[i].cash));
  }
