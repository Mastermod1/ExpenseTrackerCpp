    ...
    : viewStateFactory(viewStateFactory), height(height), width(width)
{
    ...
    set_menu_sub(menu, derwin(window, winSize->y - 5, 38, 4, 1));
...
std::shared_ptr<IViewState> DisplayDatabaseView::nextState(TuiView& view)
{
    ...
    {
	strRows.push_back(dataRows[i].name + "  " + dataRows[i].date);
    }
    ...
    set_menu_format(menu, winSize->y - 5, 1);
...
{
    // free_menu(menu);
 //    for (int i = 0; i < DISPLAY_MENU.size; ++i)
	// free_item(items[i]);
